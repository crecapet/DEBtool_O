function [par, U, Ep] = get_pars_t(p,w,par0)
  ...
  %% Bas Kooijman, 2006/12/15, modified 2007/08/07, 2008/12/08
  %% like get_pars_s, but
  %%   with stage transitions at fixed amounts of structure
  %%   with several food levels
  ...
  %% for transfer to fnget_pars_s, dget_auh, dget_lh_p
  ...
  if exist('par0', 'var') == 1
    ...
    if nr ~= 8
      par = []; U = []; Ep = [];
      fprintf('numbers of initial parameters not equal to 8\n');
      return;
    end
    par = par0; kapR = par(2,1); par([2 4],:) = []; % remove kapR and kJ
    if nc == 1
      par = [par, ones(size(par,1),1)];
    end
  else
