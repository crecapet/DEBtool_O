function R = scale (z,j)
  %% created 2000/11/02 by Bas Kooijman, modified 2009/02/03
  %% run pars_animal to fill globals
  %% body-size-scaling for 'animal'
  %% input: vector of zoom factors,
  %%   optional scalar with variable identification to reduce computation time
  %% output: matrix with the zoom factor in the rows and in the columns:
  ...
  global L_d L_m L_T l_T y_E_X jT_X_Am JT_E_Am R_m JT_X_Am JT_E_Am 
  global d_O w_O vT n_O n_M eta_O 
  global pT_M pT_Am K k kT_J kT_M
  global kap kap_R E_G m_Em hT_a s_G V_Hb v_Hb v_Hp U_Hb U_Hp
