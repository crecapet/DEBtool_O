function [par, aLM] = get_pars_u(q, p)
  ...
  %% notice that surface linked somatic maintenance costs are not included
  %%
  %% aLM: (4,4)-matrix with 
  %%   age a, length L, mass of reserve M_E, mass of structure M_V
  %%   at age 0, at birth, at puberty, at infinity
  %%
  %% notice that all quantities that involve length depend on the shape
  %%   unless if chosen for volumetric lengths
  ...
  ap = aUL(2,1); Up = aUL(2,2); Lp = aUL(2,3);
