%% mydata_get_ts and mydata_get_tj
...
%% set parameters and f
  ...
  g   = 8.3;  % [E_G] v/ kap {p_Am}, energy investment ratio 
  k   = .5;    % k_J/ k_M, ratio of maturity and somatic maintenance rate coeff
  lT  = .0;   % {p_T}/[p_M]Lm,  scaled heating length 
  vHb = 1e-3; % v_H^b = U_H^b g^2 kM^3/ (1 - kap) v^2; U_B^b = M_H^b/ {J_EAm}, mat at birth: start of feeding
  vHs = .5e-2; % v_H^s = U_H^s g^2 kM^3/ (1 - kap) v^2; U_B^s = M_H^s/ {J_EAm}, mat at settlemen: start of acceletation
  vHj = .2e-1; % v_H^j = U_H^j g^2 kM^3/ (1 - kap) v^2; U_B^j = M_H^j/ {J_EAm}, mat at metamorphosis: end of acceleration
  vHp = .8e-1;  % v_H^p = U_H^p g^2 kM^3/ (1 - kap) v^2; U_B^p = M_H^p/ {J_EAm}, mat at puberty: ceasing of maturation, start of reproduction
