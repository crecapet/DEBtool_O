...
switch 7 % change this to other numbers to run the different cases
  case 1 % single food level
    ...
    [q U] = iget_pars_g(p, f); % p = [VHb g kM v]; q = [f Lb Li ab rB]
    %% now obtain parameters from quantities
    [pn Un] = get_pars_g(q);   % q = [f Lb Li  ab rB]; pn = [VHb g kM v];
  ...
  case 6 % single food level: growth and reprod data
  ...
  case 7 % two food levels, no age at birth "data"
    %%   stage transitions not at fixed structure
    %% first obtain easy-to-measure quantities from parameters
    %%   at 2 food levels
    p = [.7; .95; 2; .01; .1; 2.5; .1; 1.5]; % p: [kap kapR g kJ kM v Hb Hp]
    [q U] = iget_pars_s(p, [1; .8; .6; .4]); % q: [f, Lb, Lp, Li, rB, Ri, kapR]
    %% now obtain parameters from quantities
