...
global g k % for plotting (tranfer to dhul)

f = 1; % scaled functional response
g = 6; k = 2; kap = .8; uHb = .001; vHb = uHb/ (1 - kap);
...
%% get lb via Newton Raphson on y(x_b) = y_b with lsode
...
printf('      get_lb    get_lb1   get_lb2   get_tul \n');
...
%% prepare plotting
  ...
  l0 = 1e-6;
  uH0 = (1 - kap) * uE0 * l0^2 * (g + l0)/ (uE0 + l0^3)/ k;
  hul_0 = [uH0; uE0; l0];
  hul_t = lsode('dhul', hul_0, t);
  uH = hul_t(:,1); uE = hul_t(:,2); l = hul_t(:,3);

  uH01 = (1 - kap) * uE01 * l0^2 * (g + l0)/ (uE01 + l0^3)/ k;
  hul1_0 = [uH01; uE01; l0];
  hul1_t = lsode('dhul', hul1_0, t1);
  uH1 = hul1_t(:,1); uE1 = hul1_t(:,2); l1 = hul1_t(:,3);

  uH02 = (1 - kap) * uE02 * l0^2 * (g + l0)/ (uE02 + l0^3)/ k;
  hul2_0 = [uH02; uE02; l0];
  hul2_t = lsode('dhul', hul2_0, t2);
  uH2 = hul2_t(:,1); uE2 = hul2_t(:,2); l2 = hul2_t(:,3);

  uH03 = (1 - kap) * uE03 * l0^2 * (g + l0)/ (uE03 + l0^3)/ k;
  hul3_0 = [uH03; uE03; l0];
  hul3_t = lsode('dhul', hul3_0, t3);
  uH3 = hul3_t(:,1); uE3 = hul3_t(:,2); l3 = hul3_t(:,3);
