function [tMd aA_out aD_out aG_out] = ...
  ...
  %% [tMd aA_out aD_out aG_out] = isotope(tTXd_, Md, p, n, aA_, aD_, aG_, odds)
  %% created by Bas Kooijman 2008/02/10, modified 2008/03/17
  %% tTXd_: (n,8)-matrix with cols (* = chem elements C,H,O,N)
  %%     time, temp, food dens X, del_*X, del_OO
  %% Md: 13-vector with values for time t = 0
  %%     res M_E(0), struc M_V(0), mat M_H(0), oto M_OD(0), oto M_OG(0)
  %%     del_*E(0), del_*V(0)
  %% p: 20-vector with DEB-parameters (see below)
  ...
  Y_MO = - inverse(n_M) * n_O; % neglect mass in otoliths
  ...
  if prod(sum(odds ~= 1,1) > 1)
   tMd = [];
   return
 end

  t = tTXd(1:nt,1); % time points
  tMd = [t, lsode('isotope_flux', Md, t)];
