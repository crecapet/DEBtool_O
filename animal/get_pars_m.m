function d = get_pars_m(par, x)
  %% d = get_pars_m(par, x)
  %% parameters for foetal development
  %% par: (19)-vector with parameters (see below)
  %% x: independent variable, not used
  %% data: (29,1)-matrix with parameters plus quantities
  %%   1 ab    % d, age at birth
...
u_Hp = U_Hp * g^2 * kT_M^3/ vT^2;% -, scaled maturity density at puberty  
  ...
  t_m = 365 * 80 * kT_M; a_m = t_m/ kT_M; % d, mean life span
