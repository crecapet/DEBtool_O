function f = fomortph (p, t, cpH)
  %% f = fomortph (p, t, cpH)
  %% ceated 2002/01/09 by Bas Kooijman
  ...
  %% application is illustrated in script-file mydata_fomortph
  ...
  f  = (t1 * (c .* b./ k)) .* max(0, e.^(-t1 * (t0 .* k)) - e.^(-t * k)) - ...
       (t1 * (b .* max(0, c - c0))) .* max(0, t*c1 - t1*t0);
  f  = min(1, e.^f).* (e.^(-h * t) * c1); % survival probability
