function shbatch1 (j)
  ...
  Xt = lsode ('dbatch1', X0, t);           % integrate state variables

  clf; hold on; gset nokey;
  ...
  if exist('j', 'var') == 1 % single-plot mode
    ...
    subplot(2, 2, 1); clf;
    gset title 'ammonia & excreted N-reserve';
    xlabel('time, d'); ylabel('ammonia, M');
    plot(t, Xt(:,1), 'b', t, Xt(:,2), '.b');

    subplot(2, 2, 2); clf;
    gset title 'phospate & excreted P-reserve';
    xlabel('time, d'); ylabel('phosphate, M');
    plot(t, Xt(:,3),'r', t, Xt(:,4), '.r');

    subplot(2, 2, 3); clf;
    gset title 'structure density';
    xlabel('time, d'); ylabel('structure, M');
    plot(t, Xt(:,7), 'g');

    subplot(2, 2, 4); clf;
    gset title 'reserve density';
    xlabel('time, d'); ylabel('res dens, mol/mol');
    plot(t, Xt(:,5), 'b', t, Xt(:,6), 'r');
  end
