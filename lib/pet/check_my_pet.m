...
function check_my_pet(speciesnms)
  ...
  % adapted for Octave 2019/03/21 by Charlotte Recapet
...
auxDataTypes = fieldnames(auxData);
txtDataFields = fieldnames(txtData);
weightFields = fieldnames(weights);
...
for i = 1:size(auxDataTypes, 1)
  ...
  auxDataFields = fieldnames(auxData.(currentAuxDataType));
...
if sum(strcmp(weightFields, 'psd'))
  psdWeightFields = fieldnames(weights.psd);
...
if sum(strcmp(txtDataFields, 'units'))
  unitsFields = fieldnames(txtData.units);
  ...
  if psdexist
    if sum(strcmp(unitsFields, 'psd'))
      psdUnitsFields = fieldnames(txtData.units.psd);
  ...
  for i = 1:length(auxDataTypes)
    if sum(strcmp(unitsFields, auxDataTypes(i)))
      currentAuxDataType = auxDataTypes{i};
      auxDataFields = fieldnames(auxData.(currentAuxDataType));
      auxDataUnitsFields = fieldnames(txtData.units.(currentAuxDataType));
...
if sum(strcmp(txtDataFields, 'label'))
  labelFields = fieldnames(txtData.label);
  ...
  if psdexist
    if sum(strcmp(labelFields, 'psd'))
      psdLabelFields = fieldnames(txtData.label.psd);
  ...
  for i = 1:length(auxDataTypes)
    if sum(strcmp(labelFields, auxDataTypes(i)))
      auxDataFields = fieldnames(auxData.(currentAuxDataType));
      auxDataLabelFields = fieldnames(txtData.label.(currentAuxDataType));
...
if isfield(metaData, 'facts')
  factsFields = fieldnames(metaData.facts);
else
  factsFields = {};
end
...
if isfield(metaData, 'discussion')
  discussionFields = fieldnames(metaData.discussion);
else
  discussionFields = {};
end

if isfield(metaData, 'bibkey')
  factsDiscussionBibkeys = fieldnames(metaData.bibkey);
else
...
if sum(strcmp(txtDataFields, 'bibkey'))
  bibkeyFields = [fieldnames(txtData.bibkey); factsDiscussionBibkeys];
...
if sum(strcmp(fieldnames(metaData), 'biblist'))
  biblistFields = fieldnames(metaData.biblist);
  ...
  bibkeyMarker = length(fieldnames(txtData.bibkey));
  for i = 1:length(bibkeyFields)
    if i > bibkeyMarker
...
if sum(strcmp(parFields, 'free'))
  ...
  freeFields = fieldnames(par.free);
...
if sum(strcmp(txtParTypes, 'units'))
  unitsFields = fieldnames(txtPar.units);
...
if sum(strcmp(txtParTypes, 'label'))
  labelFields = fieldnames(txtPar.label);
...
prdDataFields = fieldnames(prdData);
...
if sum(strcmp(prdDataFields, 'psd'))
  ...
  prdpsdFields = fieldnames(prdData.psd);
  prdpsdexist = 1;
else
  prdpsdexist = 0;
end
