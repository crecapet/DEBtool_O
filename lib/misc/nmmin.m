function [q, info] = nmmin(func, p, varargin)
  %% [q, info] = nmmin(func, p, ...)
  %% created: 2001/08/20 by Bas Kooijman
  %% find min using Nelder Mead's simplex method
  %% function [q, info] = nmmin(func, p, ...)
  ...
  func_evals = n+1;
  if report == 1
    printf(['step ', num2str(itercount), ' value ', num2str(min(fv)), '-', ...
	    num2str(max(fv)), ' ', how, '\n']);
  end
  info = 1;
  ...
  func_evals = func_evals+1;
   ...
   elseif itercount >= max_step_nr 
     if report > 0
       printf(['No convergences with ', num2str(max_step_nr), ' steps\n']);
     end
     info = 0; 
   else
     if report > 0
