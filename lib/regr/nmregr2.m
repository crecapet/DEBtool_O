function [q, info] = nmregr2(func, p, x, y, Z, W)
  %% [q, info] = nmregr2(func, p, x, y, Z, W)
  %% created: 2001/08/20 by Bas Kooijman; modified 2007/04/04
  ...
  n = length(index); % n is number of parameters that must be iterated
  if (n == 0)
  ...
  func_evals = n + 1;
  if report == 1
    printf(['step ', num2str(itercount), ' ssq ', num2str(min(fv)), '-', ...
	    num2str(max(fv)), ' ', how,'\n']);
  end
  info = 1;
