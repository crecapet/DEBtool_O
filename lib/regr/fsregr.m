function [q, info] = fsregr(func, p, varargin)
  ...
  %% par: matrix like p, but with least squares estimates
  ...
  [q(index,1), info] = fsolve('fsdregr', par(index,1));
  
  if info ~= 1
    printf('no convergence to requested tolerance \n');
  end
