function [q, info] = nmvcregr2(func, p, x, y, Z, W)
  ...
  %% created: 2004/04/05 by Bas Kooijman; corrected 2005/02/18
  %% calculates least squares estimates using Nelder Mead's simplex method
  %%    similar to nmregr2, but standard deviation proportional to mean
  ...
  func_evals = func_evals+1;
   ...
   if report == 1
     printf(['step ', num2str(itercount), ' ln div ', num2str(min(fv)), '-', ...
	     num2str(max(fv)), ' ', how, '\n']);
   end  
   end   % while
