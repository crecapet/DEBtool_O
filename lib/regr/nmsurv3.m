function [q, info] = nmsurv3(func, p, t, x, y, Z)
  %% [q, info] = nmsurv3(func, p, t, x, y, Z)
  %% created: 2006/03/07 by Bas Kooijman; modified 2006/05/28
  %% calculates maximum likelihood estimates using Nelder Mead's simplex method
  %%    similar to scsurv3, but slower and a larger bassin of attraction
  ...
  D = reshape(Z-[Z(2:nt,:);zeros(1,nxy)], ntxy, 1);
  ...
  func_evals = n + 1;
  if report == 1
    printf(['step ', num2str(itercount), ' dev ', num2str(min(fv)), '-', ...
	    num2str(max(fv)), ' ', how,'\n']);
  end
  info = 1;
  ...
  func_evals = func_evals + 1;
   ...
   if func_evals >= max_fun_evals
     if report > 0
       printf(['No convergences with ', ...
		num2str(max_fun_evals), ' function evaluations\n']);
     end
     info = 0;
   elseif itercount >= max_step_number 
     if report > 0
       printf(['No convergences with ', num2str(max_step_number), ' steps\n']);
     end
     info = 0; 
   else
     if report > 0
