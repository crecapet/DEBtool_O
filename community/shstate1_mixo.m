function shstate1_mixo (j)
  %% shstate1_mixo (j)
  %% created: 2001/09/03 by Bas Kooijman, modified 2009/02/20
  %% equilibrium_plots for 'mixo'
  %% Calls for state1_mixo, gstate1_mixo, findstate1_mixo
  ...
  x = state1_mixo(CTOT, NTOT); x = gstate1_mixo(x); xx = gstate1_mixo(x);
  XC = zeros(nC, 6); XC(nC, :) = x'; i = nC;

  while 0 < prod(x > 0) & (i > 1) % as long as all compounds positive
    i = i - 1; Ctot = C(i);
    x = gstate1_mixo(x);			  
    XC(i,:) = x';
  end
