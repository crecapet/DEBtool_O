function [X info]= gstate3_mixo (X_t)
  ...
  %% assume C limitation
  XC = zeros(8,1);
  [XC([1 3 4 6 7 8]) infC] = fsolve('findstatec3_mixo',X_t([1 3 4 6 7 8]));
  ...
  %% assume N limitation
  XN = zeros(8,1);
  [XN([2 3 4 6 7 8]) infN] = fsolve('findstaten3_mixo',X_t([2 3 4 6 7 8]));
  ...
  elseif (0 < prod(XC > 0)) && (infC == 1)
    X = XC; % C limitation
    info = 'C';
  elseif (0 < prod(XN > 0)) && (infN == 1)
    X = XN; % N limitation
    info = 'N';
  else % no positive solutions
    printf (['No pos. equilibrium for Light = ', num2str(j_L_F), ...
	     '; Ctot = ', num2str(Ctot), ...
	     '; Ntot = ', num2str(Ntot), '\n']);
