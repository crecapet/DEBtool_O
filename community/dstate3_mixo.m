function dX = dstate3_mixo (X_t, t)
  %% dX = dstate3_mixo (X_t, t)
  %% routine called from shtime3_mixo
...
k_E k_M kap_EC kap_EN h n_NV n_NE n_NEN
  ...
  EC=X_t(7); EN=X_t(8); m_E = E/V; m_EC = EC/V; m_EN = EN/V;
  ...
  j_N_E = y_NEN * j_EN_E;
  j_N_G = - n_NV * j_V_G - n_NE * j_E_G - n_NEN * j_EN_G;
  j_N_M = - n_NE * j_E_M - n_NEN * j_EN_M;
  j_N_H = - y_NEN * j_EN_H;

  dX = zeros(8,1); % create 8-vector for output

  %% change in state variables 
  dX(1) = V * (j_C_AA + j_C_AHV + j_C_AHE + j_C_E + j_C_G + j_C_M + j_C_H);
				            % DIC 
  dX(2) = V * (j_N_AA + j_N_AHV + j_N_AHE + j_N_E + j_N_G + j_N_M + j_N_H);
				            % DIN
  dX(3) = V * (j_DV_AHV + j_DV_H);          % DV, V-detritus
  dX(4) = V * (j_DE_AHE + j_DE_H);          % DE, E-detritus
  dX(5) = V * (j_V_G + j_V_H);              % V, Structure 
  dX(6) = V * (j_E_AHV + j_E_AHE + j_E_G + ...
	     j_E_M + j_E_H);    % E, Reserve
  dX(7) = V * (j_EC_AA - j_EC_E + j_EC_G + j_EC_M + j_EC_H);
				% EC, C-reserve
  dX(8) = V * (j_EN_AA - j_EN_E + j_EN_G + j_EN_M + j_EN_H);
				% EN, N-reserve
