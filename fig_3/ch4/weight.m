function w = weight(p, t, n)
  ...
  for j = 1:n
    ...
    while T0 < t(nt)
      T1 = min(t(nt) + 1e-4, T0 + exponential_rnd(lab0)); f = 0; % starving mode
      iu = sum(t < T1); id = iu - il;
      if iu > il
	    T = [T0; t((il+1):iu); T1];
        el = lsode('growthrt', el0, T); el(1,:) = [];
        w((il+1):iu, j) = el(1:id, 2) .* (1 + d * el(1:id, 1)) .^ (1/3);
	    el0 = el(id + 1,:);
      else % no time points between switch points
	    T = [T0; T1];
        el = lsode('growthrt', el0, T); el0 = el(2,:);
      end
      
      T0 = T1; il = iu;  
      T1 = min(t(nt) + 2e-4, T0 + exponential_rnd(lab1)); f = 1; % feeding mode
      iu = sum(t < T1); id = iu - il;
      if iu > il
	    T = [T0; t((il+1):iu); T1];
        el = lsode('growthrt', el0, T); el(1,:) = [];
