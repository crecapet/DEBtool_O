...
%% unit of time 1/kM
lab0 = 11.666; % prob rate from starving to feeding
lab1 = 5;      % prob rate from feeding to starving
...
mw = .01 * sum(w, 2); tw = [t, mw];  % time and mean weight
cv = sqrt(.01 * sum(w .* w, 2) - mw .* mw) ./ mw; % variation coeff
