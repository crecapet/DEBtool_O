...
plot(log10(V(1:5,1)), V(1:5,2),'og', log10(V(6,1)), V(6,2), 'or')

subplot(2,3,2)
%% gset output 'carbr.ps'
title('sodium bromide');
xlabel('10 log mg Br/l');
ylabel('1000 cells (ind.d)^-1')
plot(log10(BR(:,1)), BR(:,2),'og')

subplot(2,3,3)
%% gset output 'cardmq.ps'
title('2,6-dimethylquinoline');
xlabel('10 log mg DMQ/l');
ylabel('1000 cells (ind.d)^-1')
plot(log10(DMQ(:,1)), DMQ(:,2),'og')

subplot(2,3,4)
%% gset output 'carcr.ps'
title('potassium dichromate');
xlabel('10 log mg K2Cr2O7/l');
ylabel('1000 cells (ind.d)^-1')
plot(log10(CR(1:4,1)), CR(1:4,2),'og', log10(CR(5:6,1)), CR(5:6,2), 'or')

subplot(2,3,5)
%% gset output 'caraa.ps'
title('9-aminoacridine');
xlabel('10 log mg AA/l');
ylabel('1000 cells (ind.d)^-1')
plot(log10(AA(1:5,1)), AA(1:5,2),'og',log10(AA(6,1)), AA(6,2),'or')

subplot(2,3,6)
%% gset output 'carcol.ps'
title('colchicine');
xlabel('10 log mg Col/l');
ylabel('1000 cells (ind.d)^-1')
plot(log10(COL(1:4,1)), COL(1:4,2),'og', log10(COL(5,1)), COL(5,2),'or')
